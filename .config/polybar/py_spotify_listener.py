...
def event_handler(*args, **kwargs):
    ...
    if data[1]['PlaybackStatus'] == "Playing":
        ...
        """ Send IPC hook 2 to the mainbar bar pid for module playpause """
        with open("/tmp/ipc-mainbar", "w") as text_file:
            print("hook:module/playpause2", file=text_file)
    if data[1]['PlaybackStatus'] == "Paused":
        print("Music is paused.")
        """ Send IPC hook 3 to the mainbar bar pid for module playpause """
        with open("/tmp/ipc-mainbar", "w") as text_file:
            print("hook:module/playpause3", file=text_file)
    """ Send IPC hook 2 to the mainbar bar pid for module spotify """
    with open("/tmp/ipc-mainbar", "w") as text_file:
